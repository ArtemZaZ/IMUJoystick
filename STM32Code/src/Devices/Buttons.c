...
void ButtonsInitialize(void)
{
  ...
  buttonList[butCounter].buttonsGPIO -> MODER &= ~(0x3 << 2 * buttonList[butCounter].pin);  // Очищаем moder(input mode)
  buttonList[butCounter].buttonsGPIO -> PUPDR &= ~(0x3 << 2 * buttonList[butCounter].pin);  // Очищаем pupdr
  buttonList[butCounter].buttonsGPIO -> PUPDR |= (0x2 << 2 * buttonList[butCounter].pin);  // pull-down 
  butCounter++;  
#endif
